...
def app():
    ...
    first_null_index = df[(df['Tellisale_Agent'] == 'Ao Choene') & (df['Customer_Contacted'].isnull())].index.min()
    if  pd.notna(first_null_index):
        ...
        with col1:
           ...
           #Agent_Feedback = st.text_input(label="Agent Feedback", value=row_values["Agent_Feedback"])
           #Agent_Feedback = st.text_input(label="Comments")
        with col2:
